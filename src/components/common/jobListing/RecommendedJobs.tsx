  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const response = await api.get<{
        ...
        `user/recommended?page=${page}&title=${
          debouncedInput?.title || ""
        }&experienceLevel=${debouncedInput?.experienceLevel || ""}&industry=${
          debouncedInput?.industry || ""
        }&jobType=${debouncedInput?.jobType || ""}`
      );

      setJobs((prev) =>
        page === 1 ? response.data.jobs : [...prev, ...response.data.jobs]
      );
    ...
    <div className="p-4 h-full overflow-y-auto bg-slate-100">
      {isLoading ? (
        ...
        <>
          ...
          {!hasMore && (
            <p className="text-center my-4">No recommended jobs available.</p>
          )}
        </>
      )}
    </div>
