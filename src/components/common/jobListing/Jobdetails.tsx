          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="relative flex-1 min-w-[280px]">
                <h1 className="text-2xl font-bold text-gray-800 mb-7">{detail?.title}</h1>
                <div className="flex items-center gap-4 mb-5 mt-4">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800 text-lg">{detail?.company?.name}</p>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  <span className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-full text-sm font-medium">{detail?.jobType}</span>
                  <span className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-full text-sm font-medium">{detail?.experienceLevel}</span>
                  {detail?.industry && (
                    <span className="px-3 py-1.5 bg-purple-50 text-purple-600 rounded-full text-sm font-medium">{detail?.industry}</span>
                  )}
                </div>
              </div>
              ...
              <div className="relative min-w-[200px]">
                <div className="flex gap-3 mb-8">
                  <button
                    ...
                    onClick={() => route.push(`/user/jobs/apply/${detail?._id}`)}
                  >
                  ...
                  {copied && (
                    <div className="w-full absolute right-0 top-12 bg-white p-2 border border-gray-200 rounded-xl shadow-lg overflow-hidden z-10">
                      <div className="w-full flex justify-center items-center">
                        <span
                          className="w-full max-w-[85%] m-2 border border-gray-200 bg-gray-50 p-2 overflow-x-auto whitespace-nowrap rounded-lg text-sm font-medium text-gray-700"
                          style={{
                            scrollbarWidth: "thin",
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                  <div className="flex flex-wrap gap-4">
                    <div className="flex flex-col">
                      <span className="text-sm text-gray-500 mb-1">Minimum</span>
                      <div className="px-4 py-2 border rounded-xl bg-white shadow-sm font-medium text-gray-800">
                        {detail?.salary?.min} <span className="text-sm text-gray-500">{detail?.salary?.rate}</span>
                      </div>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-sm text-gray-500 mb-1">Maximum</span>
                      <div className="px-4 py-2 border rounded-xl bg-white shadow-sm font-medium text-gray-800">
                        {detail?.salary?.max} <span className="text-sm text-gray-500">{detail?.salary?.rate}</span>
                      </div>
                    </div>

                    {detail?.applicationDeadline && (
                      <div className="flex flex-col ml-auto">
                        <span className="text-sm text-gray-500 mb-1">Application Deadline</span>
                        <div className="px-4 py-2 border rounded-xl bg-white shadow-sm font-medium text-red-600 flex items-center gap-2">
                          <Calendar size={16} />
                          {new Date(detail.applicationDeadline).toLocaleDateString("en-US", {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric'
                          })}
              ...
              {similarjob.map((job, index) => (
                <div key={index} className="border rounded-lg p-4 bg-white shadow-lg">
                  <div className="flex items-start justify-between">
                    <Link href={`/user/jobs/details/${job._id}`}>
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm">{job.jobType}</span>
                            <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm">{job.experienceLevel}</span>
                            <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm">{job.industry}</span>
                          </div>
                    ...
                    >
                      {savedjobs.find((item) => item.jobId._id.includes(job?._id)) ? <LuBookmarkCheck /> : <LuBookmark />}
                    </button>
                  </div>
                </div>
              ))}
