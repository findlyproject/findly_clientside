            ...
            posts.length > 0 ? (
              posts.map((item) => (
                ...
                <div className="relative flex items-center gap-3">
                  ...
                  <img
                    src={item.owner?.logo||"/profile.jpg"} // Replace with actual profile image URL
                    alt="Profile"
                    className="w-12 h-12 rounded-full border"
                  />
                  <div>
                    <h2 className="text-sm font-semibold">{item.owner?.name} • <span className="text-xs text-gray-500">You</span></h2>
                    <p className="text-xs text-gray-500">{item.owner?.IndustryType} • {formatDistanceToNowStrict (new Date(item.createdAt), { addSuffix: true })}</p>
                  </div>
  ...
  {UpdateOpen && item?._id && (
           <section className="fixed inset-0 z-10 flex items-center justify-center bg-black bg-opacity-50">
             <OutsideClickHandler onOutsideClick={() => setIsUpdateOpen(false)}>
               <UpdatePost post={item} setIsUpdateOpen={setIsUpdateOpen} />
             </OutsideClickHandler>
           </section>
         )}
 <button
      ...
      {isExpanded ? item.description : `${item.description.slice(0, MAX_LENGTH)} `}
