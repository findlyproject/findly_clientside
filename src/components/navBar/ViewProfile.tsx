...
export const ViewProfile = () => {
  ...
  return (
          ...
          <div className=" flex flex-col md:flex-row justify-between">
            <div className="mt-12 flex flex-col items-start">
              ...
              <p className="text-gray-900">
                {activeuser?.jobTitle?.map((title) => title)}
              </p>
              <p className="text-gray-900 text-sm">
                {activeuser?.location?.city}• {connections.length} connections
              </p>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="mt-2 flex flex-col justify-center md:justify-start">
                {activeuser?.education?.length ? (
                  activeuser.education.map((institute, index) => (
                    <div
                      key={index}
        ...
        <div className="mt-2 flex">
          <div>
            {activeuser?.experience && activeuser.experience.length > 0 ? (
              activeuser.experience.map((item, index) => (
    ...
    <section className="relative pt-40 pb-24">
        <img src={activeuser?.banner } alt="cover-image" className="w-full absolute top-0 left-0 z-0 h-60 object-cover"/>
        <div className="w-full max-w-7xl mx-auto space-y-5 px-6 md:px-8">
            <div className="flex items-center justify-center sm:justify-start relative z-10 mb-5">
                <img src={activeuser?.profileImage} alt="user-avatar-image" className="border-4 size-28 order-solid border-white rounded-full object-cover"/>
            </div>
            <div className="flex flex-col sm:flex-row max-sm:gap-5 items-center justify-between mb-5">
                <div className="block">
                    <h3 className="font-manrope font-bold text-4xl text-gray-900 mb-1">{activeuser?.firstName} {activeuser?.lastName}</h3>
                    <p className="font-normal text-base leading-7 text-gray-500">{activeuser?.jobLocation.city}</p>
                </div>
                <button
                    ...
                    <span
                        className="px-2 font-medium text-base leading-7 text-gray-700 transition-all duration-500 group-hover:text-indigo-600">{activeuser?.jobTitle}</span>
                </button>
            </div>
            <div className="flex flex-col lg:flex-row max-lg:gap-5 items-center justify-between py-0.5">
                <div className="flex items-center gap-4">
                    <button
                        className="py-3.5 px-5 rounded-full bg-indigo-600 text-white font-semibold text-base leading-7 shadow-sm shadow-transparent transition-all duration-500 hover:shadow-gray-100 hover:bg-indigo-700" onClick={() => router.push(`/${route}/profile/edit`)}>Edit
                        Profile</button>
                    <button
                      
                      className="py-3.5 px-5 rounded-full bg-indigo-50 text-indigo-600 font-semibold text-base leading-7 shadow-sm shadow-transparent transition-all duration-500 hover:bg-indigo-100" onClick={() => setResume(true)}>Resume upload</button>
                       <div className="flex mr-3">
                       {Array.isArray(activeuser?.connecting) &&
  activeuser.connecting.slice(0, 3).map((user, index) => (
    <Image
      key={index}
      className="border-2 border-white rounded-full h-10 w-10 -mr-4"
      src={user?.connectionID?.profileImage || "/default-profile.png"}
      width={30}
      height={30}
      alt="Profile"
    />
    ...
    <span className="flex items-center justify-center bg-white text-sm text-gray-800 font-semibold border-2 border-gray-200 rounded-full h-11 w-11">{activeuser?.connecting.length}+</span>
</div> 
                </div>
                <div className="flex flex-col md:flex-row items-center gap-6 ">
                    ...
                    <ul className="flex items-center max-sm:justify-center max-sm:flex-wrap gap-2.5">
                      {activeuser?.skills&& activeuser.skills.map((skill,index)=>(
                        <li key={index}  className="py-3.5 px-7 rounded-full bg-orange-50 font-semibold text-base leading-7 text-gray-700">{skill}</li>
                      ))}
        ...
        {activeuser?.experience.map((experience,index)=>(
                    <div key={index} className="mb-6">
                        <div className="flex justify-between flex-wrap gap-2 w-full">
                            <span className="text-gray-700 font-bold">{experience?.jobRole}</span>
                            <p>
                                <span className="text-gray-700 mr-2">{experience.companyName}</span>
                                <span className="text-gray-700">{experience.startYear}-{experience.startYear}</span>
                            </p>
                        </div>
...
<div>
      ...
      <div>
        ...
        <div className="flex space-x-1 border overflow-x-auto p-2 justify-center flex-wrap ">
          {connections.length > 0 ? (
            connections.slice(0,4).map((connect) => (
              <>
              <div
                key={connect?.connectionID?._id}
