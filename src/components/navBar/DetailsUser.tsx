...
import { setConnectionRequest, setDetailes } from "@/lib/store/features/userSlice";
...
<span>{user?.role === "premium" && <Image src={verification} width={30}height={20} alt="Verified" />}</span>
  ...
  <button className="text-primary border border-primary font-semibold bg-gray-100 py-1 px-2 rounded-full hover:bg-gray-300">
    More
  </button>
</div>
 ...
 {user?.education&&user.education.length>0?(
  ...
  <h1>no education detailes</h1>
      ...
      {user?.about?(
       ...
       <div className=" flex justify-center mt-5 "> <button className="text-center text-primary">View More</button></div>
      </div>
      ):(
        <div className="p-6 border-t">
        <h3 className="text-lg font-semibold">About</h3>
        <p className="text-gray-600">write about yourselves<span className="text-primary ml-10 cursor-pointer" onClick={()=>router.push(`/editprofile`)}>Edit Profile</span></p>
...
<div className="p-6 border-t">
  ...
  <div>
   ...
   <p className="text-gray-600">{item.companyName} • {item.startYear} - {item.endYear}</p>
    ...
    <div className="p-6 border-t">
        <h3 className="text-lg font-semibold">Experience</h3>
        <p className="text-gray-600">Add experiences<span className="text-primary ml-10 cursor-pointer" onClick={()=>router.push(`/editprofile`)}>Edit Profile</span></p>
      ...
      {user?.education&&user.education.length>0?(
        ...
        <div>
        <p className="font-bold">{user?.education?.map((item)=>item.college)}</p>
        <p className="text-gray-600">{user?.education?.map((item)=>item.qualification?item.qualification:"")}• {user?.education?.map((item)=>item.startYear)} - {user?.education?.map((item)=>item.endYear)}</p>
        </div>
        </div>
        ...
        <p className="text-gray-600">Add Education<span className="text-primary ml-10 cursor-pointer" onClick={()=>router.push(`/editprofile`)}>Edit Profile</span></p>
  ...
  <div className="p-6 border-t">
    ...
    <p className="text-gray-600">
      Add your skills{" "}
      <span
        className="text-primary ml-10 cursor-pointer"
